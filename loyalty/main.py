...
@app.get("/wallet/{user_id}", response_model=schemas.WalletOut)
def get_wallet(user_id: int, db: Session = Depends(get_db)):
    return crud.get_wallet(db, user_id)

@app.post("/wallet/{user_id}/add", response_model=schemas.WalletOut)
...
@app.post("/wallet/{user_id}/redeem", response_model=schemas.RedeemResponse)
def redeem_points(user_id: int, points: int, db: Session = Depends(get_db)):
    return crud.redeem_points(db, user_id, points)

@app.post("/wallet/{user_id}/use", response_model=schemas.RedeemResponse)
def use_points(user_id: int, points: int, db: Session = Depends(get_db)):
    return crud.redeem_points(db, user_id, points)

@app.post("/wallet/{user_id}/voucher", response_model=schemas.VoucherResponse)
