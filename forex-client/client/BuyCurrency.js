...
handleBuy = async () => {
    ...
    if (!demand) {
        this.setState({ message: 'Conversion rate not available for selected currencies' });
        return;
    }
    ...
    const { optionsFrom, optionsTo, from, to, amount, message } = this.state

    return (
    <div className="container">
        <h2>💱 Forex Exchange</h2>
        <button onClick={this.handleWallet} style={{ padding: '10px', width: '100%' }}>Wallet</button>
        <div style={{ marginBottom: '10px' }}>
        <label>From Currency:</label><br />
        <select name="from" value={from} onChange={this.handleSelectChange} style={{ width: '100%' }}>
        ...
        <select name="to" value={to} onChange={this.handleSelectChange} style={{ width: '100%' }}>
            <option value="">Select</option>
        ...
        <button onClick={this.handleBuy} style={{ padding: '10px', width: '100%' }}>Buy Currency</button>
