...
export default function PaymentProcessing() {
  ...
  useEffect(() => {
    const fetchWallets = async () => {
      try {
        const response = await axios.get(`${BASE_URL}/api/wallet/${accountId}`);
  ...
  return (
    <div className="p-4 max-w-3xl mx-auto">
      <h1 className="text-xl font-bold mb-4">Hello, {username || 'User'}</h1>
          ...
          <tbody>
            {wallets.map((wallet) => (
              <tr key={wallet.id} className="border-b">
                ...
                <td className="p-2">{new Date(wallet.created_at).toLocaleDateString()}</td>
                <td className="p-2">{new Date(wallet.updated_at).toLocaleDateString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
