...
export default function Payments() {
  ...
  return (
    <div style={{
      ...
      <h2 style={{ textAlign: 'center', marginBottom: '30px' }}>🧾 Forex Transaction Invoice</h2>

      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <tbody>
          <Row label="Invoice To" value={username} />
          <Row label="Account ID" value={accountId} />
          <Row label="From Currency (ID)" value={`${fromCurrencyId}`} />
          <Row label="To Currency (ID)" value={`${toCurrencyId}`} />
          <Row label="Exchange Rate" value={`1 : ${rate}`} />
          <Row label="Amount Paid (INR)" value={amount} />
          <Row label="Converted Amount (EUR)" value={convertedAmount.toFixed(4)} />
          <Row label="Handling Charges (5%)" value={handlingCharge.toFixed(4)} />
          <Row label="Grand Total (EUR)" value={grandTotal.toFixed(4)} />
...
function Row({ label, value }) {
  return (
    <tr>
      <td style={{ padding: '10px 0', fontWeight: 'bold', textAlign: 'left' }}>{label}</td>
